  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-6">
      <div className="bg-white rounded-xl w-full max-w-md shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-5 flex flex-col gap-3.5">
          <div>
                ...
                <div className="flex gap-4 text-sm text-stone-700">
                  {['Henting', 'Levering'].map(opt => (
                    <label key={opt} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" checked={deliveryMethod === opt} onChange={() => setDeliveryMethod(opt)} className="accent-admin-accent" />
                      {opt}
                    </label>
                  ))}
                </div>
          ...
          <div>
            ...
            <textarea ref={requestedFoodRef} className={inputClass + ' resize-none overflow-hidden min-h-16'} value={requestedFood} onChange={e => setRequestedFood(e.target.value)} />
          </div>

          <div>
            <label className={labelClass}>Melding</label>
            <textarea ref={messageRef} className={inputClass + ' resize-none overflow-hidden min-h-16'} value={message} onChange={e => setMessage(e.target.value)} />
          </div>
        </div>
