  ...
  const bgImages = (
    <div className="absolute inset-0 overflow-hidden">
      <div className="absolute inset-0 md:right-1/2">
        <Image unoptimized src={img2} alt="" fill sizes="(max-width: 768px) 100vw, 50vw" placeholder="blur" className="object-cover" />
      </div>
      <div className="absolute inset-y-0 left-1/2 right-0 max-md:hidden">
        <Image unoptimized src={img1} alt="" fill sizes="50vw" placeholder="blur" className="object-cover" />
      </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input className={f('fra_kl') + ' appearance-none'} type="time" required min="17:00" max="23:59" value={form.fra_kl} onChange={e => set('fra_kl', e.target.value)} />
                {err('fra_kl')}
              </div>
              <div>
                <label className={labelClass}>Til kl. *</label>
                <input className={f('til_kl') + ' appearance-none'} type="time" required min={form.fra_kl || '17:00'} max="23:59" value={form.til_kl} onChange={e => set('til_kl', e.target.value)} />
                {err('til_kl')}
              </div>
            </div>
